...
class Paginator {
    ...
    public function pageLinks($path = '?', $ext = null)
    {
        ...
        if ($lastpage > 1) {
            ...
            if ($this->page > 1) {
                $pagination.= "<li id='p-$prev'><a href='" . $path . "$this->instance=$prev" . "$ext' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>";
            } else {
            ...
            if ($lastpage < 7 + ($adjacents * 2)) {
                for ($counter = 1; $counter <= $lastpage; $counter++) {
                    if ($counter == $this->page) {
                        $pagination.= "<li id='p-$counter' class='active'><span>$counter <span class='sr-only'>(current)</span></span></li>";
                    } else {
                        $pagination.= "<li id='p-$counter'><a href='" . $path . "$this->instance=$counter" . "$ext'>$counter</a></li>";
                    }
                }
                ...
                if ($this->page < 1 + ($adjacents * 2)) {
                    ...
                    $pagination.= "<li id='p-$lpm1'><a href='" . $path . "$this->instance=$lpm1" . "$ext'>$lpm1</a></li>";
                    $pagination.= "<li id='p-$lastpage'><a href='" . $path . "$this->instance=$lastpage" . "$ext'>$lastpage</a></li>";
                } elseif ($lastpage - ($adjacents * 2) > $this->page && $this->page > ($adjacents * 2)) {
                    $pagination.= "<li id='p-1'><a href='" . $path . "$this->instance=1" . "$ext'>1</a></li>";
                    $pagination.= "<li id='p-2'><a href='" . $path . "$this->instance=2" . "$ext'>2</a></li>";
                    $pagination.= "<li><span style='border: none; background: none; padding: 8px;'>...</span></li>";
            ...
            if ($this->page < $counter - 1) {
                $pagination.= "<li id='p-$next'><a href='" . $path . "$this->instance=$next" . "$ext' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>";
            } else {
