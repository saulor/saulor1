...
class StringMethods {
	...
	public static function __normalize ($pattern) {
	...
	public static function match ($string, $pattern) {
		preg_match_all(self::__normalize($pattern), $string, $matches, PREG_PATTERN_ORDER);
		if (!empty($matches[1])) {
			return $matches[1];
		}
		if (!empty($matches[0])) {
	...
	public static function split ($string, $pattern, $limit = null) {
		...
		return preg_split(self::__normalize($pattern), $string, $limit, $flags);
	}
	...
	*/
	...
	/*
	* Retorna a posiчуo de uma substring numa string ou -1 caso nуo seja encontrada
	*/
