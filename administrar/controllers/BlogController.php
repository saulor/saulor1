...
class BlogController extends Controller {
	...
	public function cadastrarAction() {
		try {
			...
			$blogModel = new Blog($conexao);

			$breadcrumbs = array();
			$breadcrumbs[] = array(
				'Blog' => '?modulo=' . $this->modulo
			);
			
			if (isset($_GET["id"])) {
				$model = $blogModel->getObjetoOrFail(getVariavel('id'));
				$acao = "editar";
				$breadcrumbs[] = array(
					"Atualizar" => "" 
				);
			...
			if (count($_POST) > 0) {
				...
				$dados['slugUnidade'] = Funcoes::criaSlug($dados['unidade']);
				...
				if ($model->existe('titulo')) {
					//throw new Exception('Já existe uma postagem com esse título');	
				}
				$model->salvar();
				...
				if ($acao == 'cadastrar') {	
					$acao = 'cadastrado';
					$this->log->adicionar ('cadastrou', 'blog', $model->titulo, 
						'Usuário cadastrou postagem no blog');					
					$redirecionar = sprintf('?modulo=%s&acao=%s&id=%d', $this->modulo, 
						'cadastrar', $model->id);
				}
				else {
					$acao = 'atualizado';
					$this->log->adicionar ('atualizou', 'blog', $model->titulo, 
						'Usuário atualizou postagem do blog');
					$redirecionar = sprintf('?modulo=%s', $this->modulo);
				}

				setMensagem('info', sprintf('Post %s [%s]', $acao, $model->titulo));
			...
			$conexao->disconnect();
			setMensagem("error", 'Post não encontrado');
	...
	public function excluirAction() {
		try {
			...
			$conexao = $this->conexao->getConexao();
			$model = new Blog($conexao);
			$blog = $model->getObjetoOrFail(getVariavel('id'));
			$affectedRows = $blog->excluir();
			if ($affectedRows) {
				$this->log->adicionar ('excluiu', 'blog', $model->titulo, 
					'Usuário excluiu postagem do blog.');
				$conexao->commit();					
				setMensagem('info', 'Post excluído [' . $model->titulo . ']');
			}
		}
		catch (PermissaoException $e) {
			$this->conexao->getConexao()->disconnect();
